...
1. Клонируй (если ещё не склонирован). Если репозиторий — форк, сразу
   закрепи его как основной для `gh`:

   ```bash
   gh repo set-default owner/name
   ```

   Иначе `gh` для форка по умолчанию берёт репозиторий-родитель, а
   оркестратор зовёт `gh issue list` / `gh pr` без `-R`, полагаясь на
   каталог, — и весь цикл начнёт читать очередь и PR чужого upstream'а.
   Снаружи это выглядит как вечное «Очередь пуста ✅».
